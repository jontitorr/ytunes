export const StringsEN = {
  ...
  second: "second",
  throttled:
    "This skill is being throttled by YouTube, please try again later.",
...
export const StringsFR = {
  welcome1:
    "Bienvenue sur Youtube. Dite, par exemple, jouer une vidéo de Madonna.",
  ...
  sorryskipby:
    "Désolé, je n'ai pas compris de combien je devais avancer ou reculer.",
  sorryskipto:
    "Désolé, je n'ai pas compris de combien je devais avancer ou reculer.",
  ok: "OK",
...
export const StringsIT = {
  welcome1:
    "Benvenuto su YouTube. Dì, per esempio, riproduci video dei Beatles.",
...
export const StringsDE = {
  welcome1:
    "Herzlich willkommen bei Youtube. Was kann ich für Dich tun? Sage zum Beispiel 'spiel Videos von The Beatles'.",
  ...
  sorryskipby:
    "Entschuldigung, ich habe nicht verstanden, wie viel ich überspringen soll",
  sorryskipto:
    "Entschuldigung, ich habe nicht verstanden, wohin ich springen soll",
  ok: "OK",
...
export const StringsJA = {
  welcome1:
    "ユーチューブへようこそ.  例えば, ｢ビートルズのビデオを再生｣ と話しかけてください.",
  welcome2:
    "それとも, ｢マイケル・ジャクソンの曲をシャッフル再生｣ と話しかけてください.",
...
export const StringsPT = {
  welcome1:
    "Bem vindo ao Youtube. Diga, por exemplo, toque um vídeo dos Beatles.",
...
// Create a singleton which refers to strings from ./strings.ts. This is so we can mutate and refer to our strings from anywhere in the codebase.
let strings: typeof StringsEN & Record<string, string> = StringsEN;

/**
 * Returns a locale-specific string object, with keys relating to various events and responses.
 * @returns {Object} A locale-specific string object.
 */
const getStrings = () => strings;

/**
 * Sets the locale-specific string object.
 * @param newStrings {Object} A locale-specific string object.
 */
const setStrings = (newStrings: typeof StringsEN & Record<string, string>) => {
  strings = newStrings;
};

export { getStrings, setStrings };
